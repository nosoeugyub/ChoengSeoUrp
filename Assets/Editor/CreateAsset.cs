...
public class CreateAsset : MonoBehaviour
{
    ...
    static void SetBuildShape()
    {
        ...
        foreach (Texture2D texture in _textures)
        {
            string path = AssetDatabase.GetAssetPath(texture);
            AssetDatabase.ImportAsset(path);
            string fabPath = string.Format("{0}.prefab", path.Substring(0, path.Length - 7));
            buildingitemObject = AssetDatabase.LoadAssetAtPath(string.Format(fabPath), typeof(BuildingItemObj)) as BuildingItemObj;
